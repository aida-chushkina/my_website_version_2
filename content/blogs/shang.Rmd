...
> Let's read the Airbnb data from Shanghai first


```{r read_data}
listings <- vroom("listings.csv")
```
...
#We found that in the raw data, rows 12928-43701 are all incomplete, they are missing key variable such as longitude,latitude and price.

#Therefore, we suppose these rows are listings that are not completed nor published on Airbnb, so we decide to filter them.
...
#There is very few data on monthly and weekly price. We might not be able to conduct analysis on them
```
...
After manually checking the listings with very high price per day, we have found most of them to be:
...
And as we only want to include listings in our regression analysis that are intended for travel purposes, we decide to filter the data again with following standards:
...
listings_filtered %>% 
  ...
  geom_bar() +
    labs(y = "Number of Houses",
         ...
         subtitle= "Of course, according to Proud Pudong native in our group, Tianen Yao.") +
...
- Approximately how much time did you spend on final project: We spend approximately around 20-30 hours on the final project in total.
- What, if anything, gave you the most trouble: The data is much dirtier than we have met in the problem set. We spend lots of time tidying the data.
